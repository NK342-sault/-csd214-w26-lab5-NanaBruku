...
public class InMemoryRepository implements IRepository<ProductEntity> {
    private final List<ProductEntity> db = new ArrayList<>();

    private Long idCounter = 1L;

    @Override
    public ProductEntity save(@org.jetbrains.annotations.UnknownNullability ActionGame e) {
        if (e.getId() == null) {
            e.setId(idCounter++);
            db.add(e);
        } else {
            ProductEntity existing = findById(e.getId());
            if (existing != null) {
                int index = db.indexOf(existing);
                db.set(index, e);
            } else {
                db.add(e);
            }
    ...
    @Override
    public void save(ProductEntity pub) {
